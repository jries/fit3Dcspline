...
plotI(:,:,:,5)=imstack(:,:,:,1)/globalnorm;
tab=(uitab(tgprefit,'Title','IAB'));imageslicer(plotI,'Parent',tab)

for k=1:4
plotR(:,:,:,k)=(imstack(:,:,:,k)-PSF.PSF(:,:,:,k))/globalnorm;
end
plotR(:,:,1,:)=[];plotR(:,:,end,:)=[];
...
ypos=posall(1:3:end);
xpos=posall(2:3:end);
y4pi=ph.settings_3D.y4pi;
w=ph.settings_3D.width4pi;
% for k=1
for k=length(y4pi):-1:1
    indh=ypos>w*(k-1)+1 & ypos< w*k;
    beadtrue{k}(:,1)=ypos(indh);
    beadtrue{k}(:,2)=xpos(indh);
end
end
% end